      ...
      {ficheiros.filter(f => f.tipo === "imagem").map(f => (
        <div key={f.id} style={{ position: "relative" }}>
          <div style={{ position: "relative" }}>
            <img src={f.url} alt={f.nome} style={{ width: 150, height: 100, objectFit: "cover", borderRadius: 10, cursor: "pointer", border: "1px solid var(--os-color-border)" }} onClick={() => window.open(f.url, "_blank")} />
          ...
          <div style={{ marginTop: 8, textAlign: "center", fontSize: 13, color: "var(--os-color-muted)", maxWidth: 150, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{f.nome}</div>
        </div>
      ))}
    </div>
  );
