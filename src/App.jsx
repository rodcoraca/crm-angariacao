...
export default function App() {
  ...
  useEffect(() => {
    ...
    const { data: listener } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      // Não executar operações assíncronas do Supabase diretamente
      // dentro de onAuthStateChange. O callback do GoTrue deve terminar
      // primeiro para libertar o auth-token lock.
      window.setTimeout(() => {
        if (!isMounted) return;

        if (!hydrationInFlightRef.current) {
          hydrationInFlightRef.current = hydrateSessionFromAuth(session, {
            notifyOnExpired: event !== "SIGNED_IN"
          });
        }

        hydrationInFlightRef.current.finally(() => {
          if (isMounted) {
            hydrationInFlightRef.current = null;
          }
        });
