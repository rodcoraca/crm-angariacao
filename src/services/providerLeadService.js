...
export async function listImovirtualLeads({ privateOnly, last24Hours, district, minPrice, maxPrice } = {}) {
  let query = supabase.from(PROVIDER_LEADS_TABLE)
    .select("id, external_id, title, price, area, rooms, city, district, owner_name, is_private_owner, url, created_at_first, published_at, detected_at, short_description, source, status, raw_data")
