-- DB-061: estado manual tenant-scoped das oportunidades do Radar.
-- provider_leads permanece catálogo universal; decisões manuais vivem na junction.

ALTER TABLE public.empresa_provider_listings
  ADD COLUMN IF NOT EXISTS is_private_owner_override BOOLEAN,
  ADD COLUMN IF NOT EXISTS is_active BOOLEAN NOT NULL DEFAULT true,
  ADD COLUMN IF NOT EXISTS inactive_at TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS inactive_by UUID,
  ADD COLUMN IF NOT EXISTS inactive_reason TEXT;

CREATE INDEX IF NOT EXISTS idx_empresa_provider_listings_active
  ON public.empresa_provider_listings (empresa_id, is_active, provider_lead_id);

DROP FUNCTION IF EXISTS public.radar_get_page(UUID, INTEGER, INTEGER, JSONB);
...
AS $$
  WITH scoped AS (
    SELECT
      ...
      (
        pr.last_execution IS NOT NULL
        AND pl.detected_at >= pr.last_execution
      ) AS is_new,
      CASE
  ...
  filtered AS (
    ...
    WHERE (p_filters->>'city' IS NULL OR city ILIKE '%' || (p_filters->>'city') || '%')
      ...
      AND (
        ...
        OR (p_filters->>'estado' = 'novo' AND is_new = true AND imported = false)
      )
  )
...
AS $$
  ...
  SELECT
    ...
    COUNT(*) FILTER (WHERE imported = false AND last_execution IS NOT NULL AND detected_at >= last_execution) AS novas,
...
COMMENT ON COLUMN public.empresa_provider_listings.is_private_owner_override IS
  'Classificação manual do anunciante específica da empresa; NULL usa o valor canónico do provider_leads.';
COMMENT ON COLUMN public.empresa_provider_listings.is_active IS
  'Disponibilidade manual específica da empresa; não altera provider_active global.';
