-- DB-047: RPC específica para paginação do Radar via JOIN server-side
-- Resolve o mismatch entre a SQL direta correta e a query PostgREST do getPage().

DROP FUNCTION IF EXISTS public.radar_get_page(UUID, INTEGER, INTEGER, JSONB);

CREATE FUNCTION public.radar_get_page(
  p_empresa_id UUID,
...
COMMENT ON FUNCTION public.radar_get_page(UUID, INT, INT, JSONB)
...
GRANT EXECUTE ON FUNCTION public.radar_get_page(UUID, INT, INT, JSONB)
