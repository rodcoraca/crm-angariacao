...
| DB-071 | 2026-09-11 | Ciclo de Vida Comercial dos Leads | Normalização dos estados comerciais para `novo`, `em_contacto`, `em_acompanhamento`, `agendamento`, `proposta`, `convertido`, `nao_evoluiu` e `migrado`, com mapping explícito dos valores legados conhecidos. | 🟡 Pendente | Engenharia da Plataforma OSFlow | 0.9.4 Beta | [supabase/migrations/20260911_db071_lead_commercial_statuses.sql](../supabase/migrations/20260911_db071_lead_commercial_statuses.sql) | Mantém `import_status` da junction do Radar separado do status comercial de `leads`; constraint adicionada como `NOT VALID` para preservar históricos desconhecidos. |
